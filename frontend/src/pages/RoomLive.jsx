...
const RoomLive = () => {
  ...
  useEffect(() => {
    ...
    socket.on('user-joined', (userData) => {
      ...
      if (localStreamRef.current) callPeer(userData.socketId, localStreamRef.current)
    })
  ...
  const createPeer = (remoteSocketId) => {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) socket?.emit('webrtc-ice-candidate', { to: remoteSocketId, candidate: e.candidate })
    }
    pc.ontrack = (e) => {
      ...
      <div className="room-layout">
        ...
        <div className="room-topbar">
          ...
          <div className="topbar-right">
            ...
            <button className="btn-bar" onClick={voiceEnabled ? stopVoice : startVoice} style={{ background: voiceEnabled ? '#6366f1' : 'rgba(255,255,255,0.15)' }}>
              {voiceEnabled ? '🔊' : '🎧'}
            </button>
